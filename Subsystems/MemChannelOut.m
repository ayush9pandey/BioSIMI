...
%% SimBiology Model of membrane export through conformational changes in membrane channel
%% Exported species binds to channel and is imported according to the enzyme kinetics

function SubsystemModelObj = MemChannelOut
% clear, clc, close all
SubsystemModelObj = sbiomodel('MemChannelOut');
...
% ves.input + ves.ChannelP <-> ves.C1
% Reaction rates: k1f, k1r
% ves.C1 -> ves.ChannelP + extracell.output
% Reaction rates: k2f
Sobj_input = addspecies(ves,'input','InitialAmount',50);
...
Robj_1 = addreaction(SubsystemModelObj,'ves.input + ves.ChannelP <-> ves.C1','ReactionRate','k1f*ChannelP*input - k1r*C1');
Robj_2 = addreaction(SubsystemModelObj,'ves.C1 -> ves.ChannelP + extracell.output','ReactionRate','k2f*C1');
...
title({'Time dependence of extracellular and intracellular amount',' of species during channel-mediated export'});
legend('input','output');
%}
end